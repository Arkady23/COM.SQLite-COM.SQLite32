...
## Метод Next()
Метод обеспечивает выполнение скомпелированного запроса и возвращает резульлтат построчно. Формирование строк выполняется асинхронно. Пока вы обрабатываете результат полученой строки, в это время формируется следующая строка. Метод возвращает массив значений полей строки или null.
## Метод Eof()
